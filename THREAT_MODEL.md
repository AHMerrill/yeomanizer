...
| Browser-level data leakage | `autoComplete="off"` and `spellCheck={false}` on the editor so the browser never stores or transmits entries (autofill history, or an "enhanced" spellcheck that ships text to a server). Spell-checking is instead done **offline**: an en_US Hunspell dictionary is bundled and loaded once from our own origin, and `nspell` runs in the browser — no draft text is sent (verified by recording the network: the draft appears in zero requests). `Referrer-Policy: no-referrer`; `Permissions-Policy` disables geolocation/mic/camera and FLoC. | `App.tsx`, `format/spell.ts`, `public/spell/`, `_headers` |
